...
//                     4th is for read or insert, 0 for read, 1 for insert (readit)
//										 5th is for risk name (name)
function getrisk(type,riskid,typeid,readit,name) {
    var data=[];
    var newHTML=[];
...
//            alert (data);
            if (jQuery.parseJSON(data).length) {
              if (type=='T' && readit==0) {                                // show risks for this activity type
                newHTML='<tr><td>Risk</td><td>Before</td><td class="tabicon";>Del</td></tr>';
                $(jQuery.parseJSON(data)).each(function() {
                    newHTML+=("<tr><td><a href='javascript:void(0)' onclick='prepRisk("+this.idrisk+")>"+this.risk_name+"</a></td><td");
...
if ($sel=='acttype_edit') {
  ...
  $cnti=0;
  $query="SELECT * FROM act_type WHERE idact_type=".$_GET['id'];
...
//  if ((!$resultr)||($cntir==0)) {echo "no result";}
  ...
  echo "<div id='PgTitle'>New Activity Class</div>";
  $result=FALSE;
}
?>
<div class="centered_form">
 ...
 <div id="acttypeform" class="risk_form">
  <form action="php\acttype_save.php" class="div_form" id="acttypeform" method="post">
    <?php
     if ($sel=='acttype_edit') {
...
<!--    <a href="index.php?pg=admin&sel=acttype" id='close' title="Close" class="close">X</a> -->
  </form>
 </div>
</div>
