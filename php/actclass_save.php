...
if (isset($_POST["cancel"])) {header("Location:../index.php?pg=admin&sel=actclass");};
...
if (isset($_POST["add"])) {
  $query="INSERT INTO act_class (act_name, req_woodbead, req_special, pr, qbsi, qbsoa)
...
if (isset($_POST["update"])) {
  $query="UPDATE act_class SET act_name='$acttypename', req_woodbead='$wood', req_special='$spec', pr='$pr', qbsi='$qbsi', qbsoa='$qbsoa' WHERE idact_type='$idacttype'";
  if (mysqli_query($conaap,$query)) {echo "update record";} else {echo "error ".$query." - ".$conaap->error;}
}
mysqli_close($conaap);
header("Location:../index.php?pg=admin&sel=actclass");
?>
